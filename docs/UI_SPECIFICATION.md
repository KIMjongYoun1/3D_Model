...
### 1. 메인 스튜디오 (`/studio`)
데이터 시각화의 핵심 공간으로, 3단계 레이어 구조를 가집니다.
- **Layer 1 (Background)**: 3D Quantum Canvas (Three.js) - 전체 화면 렌더링
- **Layer 2 (Bottom Sheet)**: 다이어그램 바 - 노드 검색 및 2D 관계도(ERD) 표시
- **Layer 3 (Overlay)**: 
  - **Neural Input**: 우측 슬라이드 인 패널. 파일 업로드 및 텍스트 입력 지원
...
### 2. 3D 인터랙션
- **Auto Focus**: 새로운 데이터 로드 시 카메라가 노드 그룹 중앙으로 자동 이동
